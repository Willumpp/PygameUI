...
class TextBox(UIElement):
    def __init__(self, surface, xpos, ypos, width, height, font="Comic Sans MS", font_size=14):
        super().__init__(surface, xpos, ypos, width, height, font, font_size)
...
class TextInput(UIElement):
    def __init__(self, surface, xpos, ypos, width, height, default_text, max_characters, use_int=False, max_val=2**32, min_val=-2**32):
        ...
        self.called_event = False
        #self.require_events = True
        self.hide_text = False #Hide text using **** (useful for password fields)

        self.use_int = use_int #Determine to limit box to integers
        self.max_val = max_val #Cap numerical range of box if specified to
    ...
    #   return default text if nothing has been typed
    def get_text(self):
        if self._text == "" and self._default_text != "" and self.use_int == True:
            return int(self._default_text)
        elif self._text == "" and self._default_text != "": 
            return self._default_text
        elif self._text == "" and self.use_int == True:
            return 0
        elif self._text != "" and self.use_int == False:
            return self._text
        elif self._text != "" and self.use_int == True:
        ...
        for event in events:
            ...
            if event.type == pygame.KEYDOWN and self.selected == True:
                ...
                #Character limit
                    ...
                    continue
                #If use int mode is true and key is not a number character
                elif self.use_int == True and not (48 <= ord(event.unicode) <= 57) :
                    continue
                else:
                    #Add character to draw text
                    self._text += event.unicode

        if self.selected == False and self.use_int == True:
